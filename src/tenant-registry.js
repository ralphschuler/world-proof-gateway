...
function validateTenant(input) {
  ...
  if (typeof input.rpSigningKey !== "string" || input.rpSigningKey.length === 0) throw new Error("tenant_rp_signing_key_required");
  return input;
}
...
export class PostgresTenantRegistry {
  ...
  async create(input) {
    ...
    const result = await this.#pool.query(
      "INSERT INTO tenant_projects (id, app_id, rp_id, action, environment, allowed_origins, signal_policy, attestation, signing_key_envelope) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING id, app_id, rp_id, action, environment, allowed_origins, signal_policy, status, created_at",
      [tenant.id, tenant.appId, tenant.rpId, tenant.action, tenant.environment, JSON.stringify(tenant.allowedOrigins), tenant.signalPolicy, tenant.attestation ? JSON.stringify(tenant.attestation) : null, this.#vault.seal(tenant.rpSigningKey)],
    );
    const row = result.rows[0];
