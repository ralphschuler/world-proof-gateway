...
import { configuredOwnerNullifiers, createOwnerAuth, enrollConfiguredOwners } from "./owner-auth.js";
...
const ownerNullifiers = !demoMode ? configuredOwnerNullifiers(process.env.GATEWAY_OWNER_NULLIFIERS) : [];
if (!demoMode) await enrollConfiguredOwners(store, ownerNullifiers);
...
const ownerAuth = !demoMode ? createOwnerAuth({ store, appId: process.env.GATEWAY_OWNER_APP_ID, rpId: process.env.GATEWAY_OWNER_RP_ID, signingKey: process.env.RP_SIGNING_KEY, environment: process.env.GATEWAY_OWNER_ENVIRONMENT || "production", sessionSecret: process.env.GATEWAY_SESSION_SECRET, ownerNullifiers }) : null;
